...
// FUNSI UNTUK MENAMPILKAN SEMUA DATA POST
...
// FUNSI UNTUK MENAMBAH POSTINAGN BARU
export async function addPost(titile, authorId, category, image, content) {
    try {
        await addDoc(collaction(db,'posts'),{
            titile, authorId, category, image, content
        })
    } catch(error) {
        console.error('Error data Post',error)
        throw error 
